...
function Inventory(props) {
    const deleteFoodItem = (itemId) => {
        axios.delete(`https://protected-dawn-61147-56a85301481c.herokuapp.com/fooditem/${itemId}`)
            .then(res => console.log("Item deleted successfully", res.data))
    ...
    return (
        <div>
            <div>
                ...
                <button onClick={() => deleteFoodItem(props.fooditem.id)} className="btn btn-outline-danger my-2 mx-2" style={{'borderRadius':'50px'}}>Delete</button>
            </div>
            <hr />
        </div>
    );
